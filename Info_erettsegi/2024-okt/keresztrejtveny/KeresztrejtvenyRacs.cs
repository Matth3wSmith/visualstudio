                ...
                for (int k = 0; k < Racs.GetLength(1); k++)
                {
                    ...
                    if (Racs[i, k] == '-')
                    {
                        ...
                        if ((Racs.GetLength(0) > i + 1 && Racs.GetLength(1) > k + 1))
                        {
                            ...
                            if (Racs[i, k + 1] == '-' && vizszintEleje)
                            {
                                Sorszamok[i, k] = szamlalo;
                                szamlalo++;
                                vizszintEleje = false;
                            }
                            //Függőlegesen szó kezdete-e
                            //Többi sorban van-e függőleges kezdődő szó, megnézi hogy nincs-e bent már egy szóban //Első sorban van-e függőlegesen szó
                        ...
                        else if (Racs.GetLength(0) > i + 1 && (i - 1 >= 0 && Racs[i - 1, k] != '-' && Racs[i + 1, k] == '-') || (i - 1 == -1 && Racs[i + 1, k] == '-'))
                        {
                            Sorszamok[i, k] = szamlalo;
                            szamlalo++;
                        }
                        else if(Racs.GetLength(1) > k + 1 && Racs[i, k + 1] == '-' && vizszintEleje)
                        {
                            Sorszamok[i, k] = szamlalo;
                            szamlalo++;
                            vizszintEleje = false;
